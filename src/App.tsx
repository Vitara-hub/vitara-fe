...
export default function App() {
  const { isAuthenticated, isAuthLoading, isDarkMode, toggleDarkMode, veeHealth, initAuth, user } = useStore();
  ...
  const hasActiveSession = isAuthenticated && Boolean(user) && hasAuthTokens();
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (protectedViews.has(nextView) && !hasActiveSession) {
  ...
  }, [hasActiveSession, navigateToView]);
  ...
  }, [hasActiveSession, isAuthLoading, navigateToView]);
  ...
  const renderView = (): ReactNode => {
    if (protectedViews.has(currentView) && !hasActiveSession) {
    ...
    <>
      ...
      <div className={`h-[100dvh] w-full flex flex-col overflow-hidden`}>
        <div className="flex-1 flex w-full min-h-0 bg-[#FAF9F6] dark:bg-[#121413] transition-colors duration-300 font-sans text-[#2B4B3D] dark:text-stone-100 overflow-hidden">
          ...
          {hasActiveSession && currentView !== 'landing' && currentView !== 'login' && (
          ...
          <div className="flex-1 flex flex-col relative min-w-0 overflow-hidden">
            ...
            {hasActiveSession && currentView !== 'landing' && currentView !== 'login' && !isKeyboardVisible && (
